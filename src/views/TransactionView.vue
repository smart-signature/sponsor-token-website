<template>
  <div>
    ...
    <table id="payouts" class="table table-striped table-sm table-bordered"
      ...
      <thead>
        ...
        <tbody>
          <tr class="txs" v-for="data in datas"
            v-on:click="clickList(data)" v-on:mouseover="overList()">
              <td>{{data.owner}}</td>
...
export default {
  ...
  methods: {
    clickList(data) {
      ...
      w.location.href = `https://ropsten.etherscan.io/tx/${data.hash}`;
    },
    overList() { console.log('dsafsda'); },
  },
  /* mounted: function() {
    // this.$http.get('https://api.etherscan.io/api?module=account&address=0x8d12a197cb00d4747a1fe03395095ce2a5cc6819&offset=100&page=1&sort=asc&apikey=FYFY6687BY19QA9TEF7NU2NAHQW6AFS5AN&action=txlist').then((response) => {
      ...
      for (var i in result) {
        ...
        data['price'] = parseInt(result[i]['gasPrice']) * parseInt(result[i]["gasUsed"])
                        / Math.pow(10,18);
